              ...
              {q.options?.map((opt, idx) => {
                ...
                return (
                  <label key={idx} className={`flex items-start space-x-2.5 cursor-pointer p-2.5 rounded-xl border text-sm transition-all ${isChecked
                    ? 'bg-[#eef2fc]/90 border-[#1e40a1]/40 text-[#2a4365] font-semibold'
                    ...
                    <input
                      ...
                      className="mt-0.5 text-[#1e40a1] focus:ring-[#1e40a1]/60"
                    />
                    <span>
                      <span className={`font-black mr-1 ${isSubmitted && isThisKey ? 'text-emerald-600' : isChecked ? 'text-[#1e40a1]' : 'text-slate-400'}`}>{optionLetter}.</span> {opt}
                    </span>
                  </label>
                );
              })}
