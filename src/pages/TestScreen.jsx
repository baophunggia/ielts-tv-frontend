          ...
          <div key={q.id} className="flex flex-col space-y-1">
            <div className="flex items-center space-x-3">
              ...
              <input 
                ...
                className={`border rounded px-3 py-1 w-24 focus:outline-none uppercase text-center font-semibold ${
                  isSubmitted 
        ...
        return (
          <div key={q.id} className="text-gray-800 leading-loose flex flex-wrap items-center bg-white p-2 rounded border border-gray-50">
            ...
            <input 
              ...
              value={answers[q.id] || ''}
              onChange={(e) => onAnswerChange(q.id, e.target.value)}
              className={`border-b-2 mx-2 w-36 px-2 focus:outline-none bg-transparent text-center font-semibold text-sm transition-all ${
                isSubmitted 
...
const TestScreen = () => {
  ...
  // Hiệu ứng đếm thời gian tự động tăng mỗi giây
  useEffect(() => {
    ...
    <div className="flex flex-col h-screen bg-gray-100 font-sans">
      <header className="bg-indigo-900 text-white p-4 shadow-md flex justify-between items-center shrink-0 z-10">
        ...
        <div className="flex items-center gap-4 text-sm">
          ...
          <span className="font-mono bg-indigo-950/50 px-3 py-1 rounded border border-indigo-800/60">
            <i className="fa-regular fa-clock mr-1 text-indigo-300"></i> {formatTime(seconds)}
          </span>

          {isSubmitted && (
            <button
              onClick={handleRetakeTest}
              className="bg-amber-600 hover:bg-amber-500 text-white px-4 py-2 rounded font-semibold transition flex items-center gap-1.5 shadow-sm"
            >
              <i className="fa-solid fa-rotate-left"></i> Làm lại bài
            </button>
          )}
          ...
          <button
            ...
            className={`px-4 py-2 rounded font-semibold transition ${
              isSubmitted ? 'bg-gray-500 text-gray-200 cursor-not-allowed' : 'bg-green-600 hover:bg-green-500 text-white shadow-sm'
            }`}
          >
          ...
          {isSubmitted && (
            <div className="mb-6 p-6 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-xl shadow-md flex justify-between items-center animate-fade-in">
              <div>
                ...
                <p className="text-sm text-green-100">Thời gian hoàn thành bài thi: <span className="font-mono font-bold bg-white/20 px-1.5 py-0.5 rounded ml-0.5">{formatTime(seconds)}</span></p>
              </div>
